...
int main( ) {
    ...
    char c = playersCount+'0';
    FILE *countInFile;
    countInFile = fopen("cntpls.txt","wt");
    fprintf(countInFile,"%d",playersCount);
    ...
    for (int i = 0; i < playersCount; i++)
    {
        ...
        FILE *jfp;
        jfp = fopen(s,"wt");
        fprintf(jfp, Jason_FORMAT_PERSON_OUT, players[i].vorname.buffer,players[i].lastname.buffer, players[i].age, players[i].id);
